...
var inProduct_order = {
    ...
    init: function () {//初始化
        ...
        inProduct_order.getUrl = parent.Common.getUrl();//获取请求接口的URL
        inProduct_order.getDataInterface = parent.Common.getDataInterface();//获取请求数据的接口
        inProduct_order.functionalInterface = parent.Common.functionalInterface();//获取操作功能的接口
        //inProduct_order.commonObj = parent.Common;//获取common工具类对象
        inProduct_order.userInfo = parent.Main.userInfo;//获取用户信息

        inProduct_order.setListContainerHeight();//设置列表容器得高度跟随浏览器大小变化
        window.onresize = function () {
            inProduct_order.setListContainerHeight();
        }
        ...
        al_i.css({ 'top': "14px", "height": "3px", "left": "55px" }).hide();
        
        parent.Main.redDot(function (data) {
            if (data[5].num > 0) {
                al_i.css({ 'top': "0px", "height": "20px" });
                al_i.html(data[5].num).show();
            }
        });
        //设置红点数值end
	
        inProduct_order.getDataList(that.pagePrams.curIndex);//加载数据
        this.GoDetail();//进入详情页

    },
    initPage: function () {//初始化分页插件
        var that=this;
        if (inProduct_order.pagePrams.isInit > 0)
            return;
        inProduct_order.pagePrams.isInit = 1;
        $(".page-box").html("");
        $(".page-box").append($("<div id=\"pagination\" class=\"page fl\"></div>"));
        $("#pagination").pagination({
            currentPage: inProduct_order.pagePrams.curIndex,
            totalPage: inProduct_order.pagePrams.totalPage,
            isShow: true,
            count: inProduct_order.pagePrams.pageSize,
            homePageText: "首页",
            endPageText: "尾页",
            prevPageText: "上一页",
            nextPageText: "下一页",
            callback: function (currIndex) {
                ...
                inProduct_order.getDataList(currIndex);
            }
        });
    },
    ...
    getDataList: function (currIndex) {
        var url = inProduct_order.getUrl["order"] + inProduct_order.getDataInterface["inProduct"];//获取全部订单接口地址
        var cosImgUrl = inProduct_order.getUrl["cosImgUrl"];
        //请求数据
        //inProduct_order.commonObj.ajax(url, {
        Common.ajax(url, {
            "userid": inProduct_order.userInfo.userid,
            "token": inProduct_order.userInfo.token,
            "roletype": inProduct_order.userInfo.roletype,
            "workflow": 1,
            "searchday": 30,
            "fromtime": "2016",
            "totime": "2018",
            "ordernumofsheet": inProduct_order.pagePrams.pageSize,
            ...
            function (data) {
                if (data) {
					...
					inProduct_order.pagePrams.curIndex = currIndex;//当前页码
                    inProduct_order.pagePrams.totalPage = Math.ceil(dataList.totalnum / inProduct_order.pagePrams.pageSize);//共多少页
 ...
 //命名空间
                ...
                inProduct_order.initPage();
                inProduct_order.initPriceRuler();
            });
    },
...
var base={
    init:function () {
        inProduct_order.getDataList(inProduct_order.pagePrams.curIndex);
    },
