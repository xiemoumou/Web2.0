...
var all_order = {
    ...
    init: function () {//初始化
        ...
        factory_ord.css({ 'top': "14px", "height": "3px", "left": "85px"}).hide();
        
        parent.Main.redDot(function (data) {
            if (data[4].num > 0) {
                factory_ord.css({ 'top': "0px", "height": "20px" });
                factory_ord.html(data[4].num).show();
            }
    ...
    getDataList: function (currIndex) {
        var url = all_order.getUrl["order"] + all_order.getDataInterface["newFactoryOrder"];//获取待接受生产订单(车间新订单)接口地址
