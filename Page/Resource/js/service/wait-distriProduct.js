...
var all_order = {
    ...
    init: function () {//初始化
        ...
        //设置红点数值
        var waitDistriProduct = $("#waitDistriProduct_i");
        
        waitDistriProduct.css({ 'top': "14px", "height": "3px" }).hide();
        
        parent.Main.redDot(function (data) {
            if (data[6].num > 0) {
                waitDistriProduct.css({ 'top': "0px", "height": "20px",'display':'inline-block' });
                waitDistriProduct.html(data[6].num).show();
            }
    ...
    getDataList: function (currIndex) {
        var url = all_order.getUrl["order"] + all_order.getDataInterface["waitDistriProduct"];//获取全部订单接口地址
            ...
            function (data) {
                if (data) {
                    ...
                    /*$.each(dataList.orderarray, function (i, item) {
                        ...
                        //按钮状态判断
                        if (orderstate == 2) {//待接单
                            btn = "<button class=\"rob\" onclick=\"parent.Main.robbery('" + orderId + "','" + customid + "',all_order.robbery)\">立即抢单</button><a class=\"desc\" href=\"javascript:parent.Main.demandDetails('" + orderId + "','" + customid + "')\">需求详情</a>";
                        }
