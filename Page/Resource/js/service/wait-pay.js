...
var all_order = {
    ...
    init: function () {//初始化
        ...
        al_i.css({ 'top': "14px", "height": "3px" }).hide();
        // al_i.html(5).show();
        // al_i.css({ 'top': "0px", "height": "20px" });
        parent.Main.redDot(function (data) {
            if (data[5].num > 0) {
                al_i.css({ 'top': "0px", "height": "20px",'display':'inline-block' });
                al_i.html(data[5].num).show();
               // al_i.css({'display':'inline-block'})
            }
    ...
    getDataList: function (currIndex) {
        var url = all_order.getUrl["order"] + all_order.getDataInterface["wait-pay"];//获取全部订单接口地址
