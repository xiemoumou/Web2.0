...
var waiting_design = {
    ...
    getDataList: function (currIndex,sortCode) {
        ...
        var url = waiting_design.getUrl["order"] + waiting_design.getDataInterface["waiting-design"];//获取待设计接口地址
            ...
            function (data) {
                if (data) {
                    ...
                    $.each(dataList.orderarray, function (i, item) {
                        ...
                        var designprice = item.price.designprice;
                        var state = item.state.designstate.msg;


                        var detial = $("<div id=\"order_" + orderId + "\"  class=\"detial\"> </div>");
                        //待设计
                        var btn = "<button class=\"rob\" onclick=\"parent.Main.designDraft('" + orderId + "','" + customid + "');\" >提交设计</button>";
                        var detialbtn = "<a class=\"desc\" href=\"javascript:parent.Main.designDraft('" + orderId + "','" + customid + "')\">需求详情</a>";

                        //急单var detiHead=$(" <div class=\"head\"><span class=\"date\">"+orderDate+"</span><span class=\"ordNum\">订单号: "+orderId+"</span><span class=\"jidan\">急单</span></div>");
                        var detiHead = $("<div class=\"head\"><span class=\"date\">" + orderDate + "&nbsp;&nbsp;&nbsp;订单号 ：" + orderId + "</span>" + detialbtn + "</div>");


                        //var detiBody = $("<div class=\"body\"><ul><li><a class=\"img\" href=\"javascript:parent.Main.designDraft('" + orderId + "','" + customid + "');\"><img onerror=\"this.src='../../images/images/mr.png'\" alt=\"图片\" src=\"" + cosImgUrl + imageSrc + "\" /></a></li><li><span class=\"describe\">" + goodsclass + " " + texturename + " " + accessoriesname + " </br> " + shape + " " + technology + " " + color + "</span></li><li><span class=\"size\">" + l + "mm＊" + w + "mm＊" + h + "mm</span></li><li><span class=\"money\">¥" + designprice + "</span></li><li><span class=\"day\">--</span></li><li><span class=\"orderState\">"+state+"</span></li><li>" + btn + "</li></ul></div>");
...
$(function () {
    ...
    $(".sort ul li").on('click',function () {
        ...
        waiting_design.pagePrams.curIndex=1;
        sessionStorage.setItem('pageIndex',waiting_design.pagePrams.curIndex);//分页保持
        ...
        if(Thname ==1){
            //console.log(Thname);
            waiting_design.getDataList(waiting_design.pagePrams.curIndex,0);//加载数据
        }else if(Thname ==2){
            //console.log(attr);
            if (attr==0){
                $(this).attr("attribute","1");
                waiting_design.getDataList(waiting_design.pagePrams.curIndex,2);

            }else if(attr==1){
                $(this).attr("attribute","0");
                waiting_design.getDataList(waiting_design.pagePrams.curIndex,1);
            }
        }else if(Thname ==3){
            if (attr==0){
                $(this).attr("attribute","1");
                waiting_design.getDataList(waiting_design.pagePrams.curIndex,4);

            }else if(attr==1){
                $(this).attr("attribute","0");
                waiting_design.getDataList(waiting_design.pagePrams.curIndex,3);
            }
        }else if(Thname ==4){
            if (attr==0){
                $(this).attr("attribute","1");
                waiting_design.getDataList(waiting_design.pagePrams.curIndex,6);

            }else if(attr==1){
                $(this).attr("attribute","0");
                waiting_design.getDataList(waiting_design.pagePrams.curIndex,5);
            }
        }
