$(document).ready(function(){
	/* when click supportBtn , suportnum + 1 */
	$("button.spitslot_supportBtn").each(function(){
		var $this = $(this);
		$this.click(function(){
			
			var txt = $this.children("em").text();
			$this.children("em").text( parseInt(txt)+ 1 );
			$this.addClass("redcol").attr("disabled",true);
		});
	});
	/* when click opposeBtn , opposenum + 1 */
	$("button.spitslot_opposeBtn").each(function(){
		var $this = $(this);
		$this.click(function(){
			
			var txt = parseInt( $this.children("em").text());
			/*if( txt <= 1){
				txt = 1;
			}*/
			$this.children("em").text( txt + 1 );
			$this.addClass("redcol").attr("disabled",true);			
		});
	});
	/* 实现粘贴图片 */
        ...
        for (var i = 0; i < fs.length; i++) {
          ...
          if (_type.indexOf('image') != -1) {
            ...
            reader.onload = function() {
                ...
                $(".feedback_cont").append(oImg); //oDiv转换为js对象调用方法
              }
              //无论成功与否都会触发
